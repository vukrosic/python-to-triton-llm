# triton_rms_norm_gelu_silu_rms_norm_gelu_softmax.py
...
def torch_rms_norm_gelu_silu_rms_norm_gelu_softmax(x, weight1, weight2, eps):
    """Standard PyTorch implementation: softmax(gelu(rms_norm(gelu(rms_norm(x)) * silu(rms_norm(x)))))"""
    ...
    # GELU activation
    x_gelu2 = torch.nn.functional.gelu(x_rms2)
    
    # Softmax
    return torch.softmax(x_gelu2, dim=-1)

# Triton kernel - fused RMS normalization + GELU * SiLU + RMS normalization + GELU + softmax
@triton.jit
def rms_norm_gelu_silu_rms_norm_gelu_softmax_kernel(
    ...
    x_scaled_gelu2 = x_rms2 * 0.7071067811865476  # 1/sqrt(2)
    x_erf_gelu2 = tl.libdevice.erf(x_scaled_gelu2)
    x_gelu2 = 0.5 * x_rms2 * (1 + x_erf_gelu2)
...
def triton_rms_norm_gelu_silu_rms_norm_gelu_softmax(x, weight1, weight2, eps=1e-6):
    ...
    # Launch kernel
    rms_norm_gelu_silu_rms_norm_gelu_softmax_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_rms_norm_gelu_silu_rms_norm_gelu_softmax(x, weight1, weight2, eps)
    triton_result = triton_rms_norm_gelu_silu_rms_norm_gelu_softmax(x, weight1, weight2, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_rms_norm_gelu_silu_rms_norm_gelu_softmax(x, weight1, weight2, eps)
            _ = triton_rms_norm_gelu_silu_rms_norm_gelu_softmax(x, weight1, weight2, eps)
