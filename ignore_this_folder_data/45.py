# triton_rms_norm_silu_softmax.py
...
def torch_rms_norm_silu_softmax(x, weight, eps):
    """Standard PyTorch implementation: softmax(silu(rms_norm(x)))"""
    # RMS normalization
...
# Triton kernel - fused RMS normalization + SiLU + softmax
@triton.jit
def rms_norm_silu_softmax_kernel(
...
def triton_rms_norm_silu_softmax(x, weight, eps=1e-6):
    ...
    # Launch kernel
    rms_norm_silu_softmax_kernel[grid](
        x, weight, output,
        n_cols, eps,
...
def test_correctness():
    ...
    torch_result = torch_rms_norm_silu_softmax(x, weight, eps)
    triton_result = triton_rms_norm_silu_softmax(x, weight, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_rms_norm_silu_softmax(x, weight, eps)
            _ = triton_rms_norm_silu_softmax(x, weight, eps)
