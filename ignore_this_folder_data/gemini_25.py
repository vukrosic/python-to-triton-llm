...
def torch_neg_sigmoid(x):
    """Standard PyTorch implementation"""
    return torch.sigmoid(-x)

# Triton kernel - fused element-wise Negation + Sigmoid
@triton.jit
def neg_sigmoid_kernel(
    x_ptr,
    output_ptr,
    n_elements,
...
def test_correctness():
    ...
    torch_result = torch_neg_sigmoid(x)
    triton_result = triton_neg_sigmoid(x)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_neg_sigmoid(x)
            _ = triton_neg_sigmoid(x)
