...
def torch_add_relu(x, y):
    """Standard PyTorch implementation"""
    return torch.relu(x + y)

# Triton kernel - fused element-wise addition + ReLU
@triton.jit
def add_relu_kernel(
...
def test_correctness():
    ...
    torch_result = torch_add_relu(x, y)
    triton_result = triton_add_relu(x, y)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_add_relu(x, y)
            _ = triton_add_relu(x, y)
