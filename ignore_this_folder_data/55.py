# triton_gelu_silu_layer_norm_silu_softmax.py
...
def torch_gelu_silu_layer_norm_silu_softmax(x, normalized_shape, weight, bias, eps):
    """Standard PyTorch implementation: softmax(silu(layer_norm(gelu(x) * silu(x))))"""
...
# Triton kernel - fused GELU * SiLU + layer norm + SiLU + softmax
@triton.jit
def gelu_silu_layer_norm_silu_softmax_kernel(
...
def triton_gelu_silu_layer_norm_silu_softmax(x, normalized_shape, weight, bias, eps=1e-5):
    ...
    # Launch kernel
    gelu_silu_layer_norm_silu_softmax_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_gelu_silu_layer_norm_silu_softmax(x, normalized_shape, weight, bias, eps)
    triton_result = triton_gelu_silu_layer_norm_silu_softmax(x, normalized_shape, weight, bias, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_gelu_silu_layer_norm_silu_softmax(x, normalized_shape, weight, bias, eps)
            _ = triton_gelu_silu_layer_norm_silu_softmax(x, normalized_shape, weight, bias, eps)
