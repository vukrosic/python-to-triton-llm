...
def torch_sub_div_mul_relu(x, y, z, w):
    \"\"\"Standard PyTorch implementation: relu((x - y) / z * w)\"\"\"
    intermediate = (x - y) / z * w
    return torch.nn.functional.relu(intermediate)

# Triton kernel - fused sub + div + mul + ReLU
@triton.jit
def sub_div_mul_relu_kernel(
...
def triton_sub_div_mul_relu(x, y, z, w):
...
def test_correctness():
    ...
    torch_result = torch_sub_div_mul_relu(x, y, z, w)
    triton_result = triton_sub_div_mul_relu(x, y, z, w)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_sub_div_mul_relu(x, y, z, w)
            _ = triton_sub_div_mul_relu(x, y, z, w)
