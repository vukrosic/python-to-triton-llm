...
# Triton kernel - fused element-wise Exponential + Logarithm
@triton.jit
def exp_log_kernel(
    x_ptr,
    output_ptr,
    ...
    result = tl.log(tl.exp(x))
    
    # Store result
    tl.store(output_ptr + offsets, result, mask=mask)

def triton_exp_log(x):
...
def test_correctness():
    ...
    x = torch.randn(1024, 1024, device='cuda', dtype=torch.float32) # No need for abs, exp(x) is always positive
    
    torch_result = torch_exp_log(x)
    triton_result = triton_exp_log(x)
