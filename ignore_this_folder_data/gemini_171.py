...
def torch_add_hardswish(x, y):
    """Standard PyTorch implementation"""
    return torch.nn.functional.hardswish(x + y)

# Triton kernel - fused element-wise Addition + Hardswish
@triton.jit
...
def test_correctness():
    ...
    torch_result = torch_add_hardswish(x, y)
    triton_result = triton_add_hardswish(x, y)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_add_hardswish(x, y)
            _ = triton_add_hardswish(x, y)
