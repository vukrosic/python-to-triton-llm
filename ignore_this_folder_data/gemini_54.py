...
# Triton kernel - fused element-wise Negation + Logarithm
@triton.jit
def neg_log_kernel(
    x_ptr, output_ptr,
    n_elements,
...
def test_correctness():
    ...
    x = torch.randn(1024, 1024, device='cuda', dtype=torch.float32) - 1.0 # Ensure negative for log(-x)
    
    torch_result = torch_neg_log(x)
    triton_result = triton_neg_log(x)
...
def benchmark():
    ...
    for M, N in sizes:
        print(f"Size: ({M}, {N})")
        x = torch.randn(M, N, device='cuda', dtype=torch.float32) - 1.0
