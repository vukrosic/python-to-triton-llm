...
def torch_log_square(x):
    """Standard PyTorch implementation"""
    return (torch.log(x)) * (torch.log(x))

# Triton kernel - fused element-wise Logarithm + Square
@triton.jit
...
def test_correctness():
    ...
    torch_result = torch_log_square(x)
    triton_result = triton_log_square(x)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_log_square(x)
            _ = triton_log_square(x)
