...
def torch_add_gelu(x, y):
    """Standard PyTorch implementation"""
    return torch.nn.functional.gelu(x + y)

# Triton kernel - fused element-wise Addition + GELU
@triton.jit
def add_gelu_kernel(
...
def test_correctness():
    ...
    torch_result = torch_add_gelu(x, y)
    triton_result = triton_add_gelu(x, y)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_add_gelu(x, y)
            _ = triton_add_gelu(x, y)
