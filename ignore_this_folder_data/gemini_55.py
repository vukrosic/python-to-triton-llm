...
def torch_square_log(x):
    """Standard PyTorch implementation"""
    return torch.log(x * x)

# Triton kernel - fused element-wise Square + Logarithm
@triton.jit
def square_log_kernel(
    x_ptr,
    output_ptr,
    n_elements,
...
def test_correctness():
    ...
    torch_result = torch_square_log(x)
    triton_result = triton_square_log(x)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_square_log(x)
            _ = triton_square_log(x)
