...
def torch_square_sqrt(x):
    """Standard PyTorch implementation"""
    return torch.sqrt(x * x)

# Triton kernel - fused element-wise Square + Square Root
@triton.jit
def square_sqrt_kernel(
    x_ptr,
    output_ptr,
    n_elements,
...
def test_correctness():
    ...
    torch_result = torch_square_sqrt(x)
    triton_result = triton_square_sqrt(x)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_square_sqrt(x)
            _ = triton_square_sqrt(x)
