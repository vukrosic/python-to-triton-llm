...
def torch_rms_norm_gelu(x, weight, eps):
    """Standard PyTorch implementation: gelu(rms_norm(x))"""
    # RMS normalization
    variance = x.pow(2).mean(-1, keepdim=True)
    x_norm = x * torch.rsqrt(variance + eps)
    x_rms = weight * x_norm
    # GELU activation
    return torch.nn.functional.gelu(x_rms)

# Triton kernel - fused RMS normalization + GELU
@triton.jit
def rms_norm_gelu_kernel(
    ...
    result = 0.5 * x_rms * (1 + x_erf)
    
    # Store result
    tl.store(output_ptr + row_start + col_offsets, result, mask=mask)

def triton_rms_norm_gelu(x, weight, eps=1e-6):
...
def test_correctness():
    ...
    torch_result = torch_rms_norm_gelu(x, weight, eps)
    triton_result = triton_rms_norm_gelu(x, weight, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_rms_norm_gelu(x, weight, eps)
            _ = triton_rms_norm_gelu(x, weight, eps)
