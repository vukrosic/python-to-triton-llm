...
def torch_mul_hardshrink(x, y, lambd):
    """Standard PyTorch implementation"""
    return torch.nn.functional.hardshrink(x * y, lambd=lambd)

# Triton kernel - fused element-wise Multiplication + Hardshrink
@triton.jit
def mul_hardshrink_kernel(
...
def triton_mul_hardshrink(x, y, lambd):
...
def test_correctness():
    ...
    torch_result = torch_mul_hardshrink(x, y, lambd)
    triton_result = triton_mul_hardshrink(x, y, lambd)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_mul_hardshrink(x, y, lambd)
            _ = triton_mul_hardshrink(x, y, lambd)
