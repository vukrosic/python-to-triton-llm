...
def torch_sub_div_layer_norm(x, y, z, normalized_shape, weight, bias, eps):
    """Standard PyTorch implementation: layer_norm((x - y) / z)"""
    intermediate = (x - y) / z
    return torch.nn.functional.layer_norm(intermediate, normalized_shape, weight, bias, eps)

# Triton kernel - fused sub + div + layer normalization
@triton.jit
def sub_div_layer_norm_kernel(
    x_ptr, y_ptr, z_ptr, output_ptr, weight_ptr, bias_ptr,
    n_cols, eps,
...
def triton_sub_div_layer_norm(x, y, z, normalized_shape, weight, bias, eps=1e-5):
    ...
    # Launch kernel
    sub_div_layer_norm_kernel[grid](
        x, y, z, output, weight, bias,
        n_cols, eps,
...
def test_correctness():
    ...
    torch_result = torch_sub_div_layer_norm(x, y, z, normalized_shape, weight, bias, eps)
    triton_result = triton_sub_div_layer_norm(x, y, z, normalized_shape, weight, bias, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_sub_div_layer_norm(x, y, z, normalized_shape, weight, bias, eps)
            _ = triton_sub_div_layer_norm(x, y, z, normalized_shape, weight, bias, eps)
