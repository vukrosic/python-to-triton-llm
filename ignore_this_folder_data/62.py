# triton_layer_norm_gelu_silu_layer_norm_gelu_softmax.py
...
def torch_layer_norm_gelu_silu_layer_norm_gelu_softmax(x, normalized_shape, weight1, bias1, weight2, bias2, eps):
    """Standard PyTorch implementation: softmax(gelu(layer_norm(gelu(layer_norm(x)) * silu(layer_norm(x)))))"""
    # First layer normalization
    x_norm1 = torch.nn.functional.layer_norm(x, normalized_shape, weight1, bias1, eps)
    
    # GELU and SiLU activations
    x_gelu1 = torch.nn.functional.gelu(x_norm1)
    x_silu1 = torch.nn.functional.silu(x_norm1)
    
    # Elementwise multiplication
    x_combined1 = x_gelu1 * x_silu1
    
    # Second layer normalization
    x_norm2 = torch.nn.functional.layer_norm(x_combined1, normalized_shape, weight2, bias2, eps)
    
    # GELU activation
    x_gelu2 = torch.nn.functional.gelu(x_norm2)
    
    # Softmax
    return torch.softmax(x_gelu2, dim=-1)

# Triton kernel - fused layer norm + GELU * SiLU + layer norm + GELU + softmax
@triton.jit
def layer_norm_gelu_silu_layer_norm_gelu_softmax_kernel(
    ...
    x_scaled_gelu1 = x_norm1 * 0.7071067811865476  # 1/sqrt(2)
    x_erf_gelu1 = tl.libdevice.erf(x_scaled_gelu1)
    x_gelu1 = 0.5 * x_norm1 * (1 + x_erf_gelu1)
    
    # Compute first SiLU: x * sigmoid(x)
    x_sigmoid_silu1 = tl.sigmoid(x_norm1)
    x_silu1 = x_norm1 * x_sigmoid_silu1
    
    # Elementwise multiplication
    combined1 = x_gelu1 * x_silu1
    
    # Second layer norm
    mean2 = tl.sum(combined1, axis=0) / n_cols
    var2 = tl.sum((combined1 - mean2) * (combined1 - mean2), axis=0) / n_cols
    ...
    x_norm2 = (combined1 - mean2) * rstd2 * weight2 + bias2
    
    # Compute second GELU: 0.5 * x * (1 + erf(x / sqrt(2)))
    x_scaled_gelu2 = x_norm2 * 0.7071067811865476  # 1/sqrt(2)
    x_erf_gelu2 = tl.libdevice.erf(x_scaled_gelu2)
    x_gelu2 = 0.5 * x_norm2 * (1 + x_erf_gelu2)
    
    # Softmax activation
    x_minus_max = x_gelu2 - tl.max(x_gelu2, axis=0)
...
def triton_layer_norm_gelu_silu_layer_norm_gelu_softmax(x, normalized_shape, weight1, bias1, weight2, bias2, eps=1e-5):
    ...
    # Launch kernel
    layer_norm_gelu_silu_layer_norm_gelu_softmax_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_layer_norm_gelu_silu_layer_norm_gelu_softmax(x, normalized_shape, weight1, bias1, weight2, bias2, eps)
    triton_result = triton_layer_norm_gelu_silu_layer_norm_gelu_softmax(x, normalized_shape, weight1, bias1, weight2, bias2, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_layer_norm_gelu_silu_layer_norm_gelu_softmax(x, normalized_shape, weight1, bias1, weight2, bias2, eps)
            _ = triton_layer_norm_gelu_silu_layer_norm_gelu_softmax(x, normalized_shape, weight1, bias1, weight2, bias2, eps)
