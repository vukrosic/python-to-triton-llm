# triton_sub_mul_div_add_silu.py
...
def torch_sub_mul_div_add_silu(x, y, z, w, v):
    \"\"\"Standard PyTorch implementation: silu((x - y) * z / w + v)\"\"\"
    intermediate = (x - y) * z / w + v
    return torch.nn.functional.silu(intermediate)

# Triton kernel - fused sub + mul + div + add + SiLU
@triton.jit
def sub_mul_div_add_silu_kernel(
...
def triton_sub_mul_div_add_silu(x, y, z, w, v):
    ...
    # Launch kernel
    sub_mul_div_add_silu_kernel[grid](
        x, y, z, w, v, output,
        n_elements,
...
def test_correctness():
    ...
    torch_result = torch_sub_mul_div_add_silu(x, y, z, w, v)
    triton_result = triton_sub_mul_div_add_silu(x, y, z, w, v)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_sub_mul_div_add_silu(x, y, z, w, v)
            _ = triton_sub_mul_div_add_silu(x, y, z, w, v)
