...
def torch_matmul_relu(x, w):
    """Standard PyTorch implementation"""
    return torch.relu(torch.matmul(x, w))

# Triton kernel - fused matmul + ReLU
@triton.jit
def matmul_relu_kernel(
    ...
    result = tl.where(accumulator > 0, accumulator, 0.0)
...
def test_correctness():
    ...
    torch_result = torch_matmul_relu(x, w)
    triton_result = triton_matmul_relu(x, w)
...
def benchmark():
    ...
    for M, K, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_matmul_relu(x, w)
            _ = triton_matmul_relu(x, w)
