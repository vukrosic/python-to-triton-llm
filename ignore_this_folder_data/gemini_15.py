...
def torch_log_reciprocal(x):
    """Standard PyTorch implementation"""
    return 1.0 / torch.log(x)

# Triton kernel - fused element-wise logarithm + Reciprocal
@triton.jit
def log_reciprocal_kernel(
    x_ptr, output_ptr,
    n_elements,
...
def test_correctness():
    ...
    torch_result = torch_log_reciprocal(x)
    triton_result = triton_log_reciprocal(x)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_log_reciprocal(x)
            _ = triton_log_reciprocal(x)
