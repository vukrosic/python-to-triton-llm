...
def torch_gelu_rms_norm(x, weight, eps):
    """Standard PyTorch implementation: rms_norm(gelu(x))"""
    x_gelu = torch.nn.functional.gelu(x)
    variance = x_gelu.pow(2).mean(-1, keepdim=True)
    x_norm = x_gelu * torch.rsqrt(variance + eps)
    return weight * x_norm

# Triton kernel - fused GELU + RMS normalization
@triton.jit
def gelu_rms_norm_kernel(
...
def triton_gelu_rms_norm(x, weight, eps=1e-6):
...
def test_correctness():
    ...
    torch_result = torch_gelu_rms_norm(x, weight, eps)
    triton_result = triton_gelu_rms_norm(x, weight, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_gelu_rms_norm(x, weight, eps)
            _ = triton_gelu_rms_norm(x, weight, eps)
