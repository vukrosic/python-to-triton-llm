...
# Triton kernel - fused element-wise Sine + Cosine
@triton.jit
def sin_cos_kernel(
    x_ptr,
    output_ptr,
    ...
    result = tl.cos(tl.sin(x))
    
    # Store result
    tl.store(output_ptr + offsets, result, mask=mask)

def triton_sin_cos(x):
...
def test_correctness():
    ...
    x = torch.randn(1024, 1024, device='cuda', dtype=torch.float32) * 2 * 3.14159 # Values for sin/cos
    
    torch_result = torch_sin_cos(x)
    triton_result = triton_sin_cos(x)
...
def benchmark():
    ...
    for M, N in sizes:
        print(f"Size: ({M}, {N})")
        x = torch.randn(M, N, device='cuda', dtype=torch.float32) * 2 * 3.14159
