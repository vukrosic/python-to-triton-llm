...
def torch_layer_norm_gelu(x, normalized_shape, weight, bias, eps):
    """Standard PyTorch implementation: gelu(layer_norm(x))"""
    x_norm = torch.nn.functional.layer_norm(x, normalized_shape, weight, bias, eps)
    return torch.nn.functional.gelu(x_norm)

# Triton kernel - fused layer norm + GELU
@triton.jit
def layer_norm_gelu_kernel(
    ...
    result = 0.5 * x_norm * (1 + x_erf)
    
    # Store result
    tl.store(output_ptr + row_offset + col_offsets, result, mask=mask)

def triton_layer_norm_gelu(x, normalized_shape, weight, bias, eps=1e-5):
...
def test_correctness():
    ...
    torch_result = torch_layer_norm_gelu(x, normalized_shape, weight, bias, eps)
    triton_result = triton_layer_norm_gelu(x, normalized_shape, weight, bias, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_layer_norm_gelu(x, normalized_shape, weight, bias, eps)
            _ = triton_layer_norm_gelu(x, normalized_shape, weight, bias, eps)
