# triton_silu_rms_norm_softmax.py
...
def torch_silu_rms_norm_softmax(x, weight, eps):
    \"\"\"Standard PyTorch implementation: softmax(rms_norm(silu(x)))\"\"\"
...
# Triton kernel - fused SiLU + RMS normalization + softmax
@triton.jit
def silu_rms_norm_softmax_kernel(
...
def triton_silu_rms_norm_softmax(x, weight, eps=1e-6):
    ...
    # Launch kernel
    silu_rms_norm_softmax_kernel[grid](
        x, weight, output,
        n_cols, eps,
...
def test_correctness():
    ...
    torch_result = torch_silu_rms_norm_softmax(x, weight, eps)
    triton_result = triton_silu_rms_norm_softmax(x, weight, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_silu_rms_norm_softmax(x, weight, eps)
            _ = triton_silu_rms_norm_softmax(x, weight, eps)
