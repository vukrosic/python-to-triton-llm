...
def torch_abs_sign(x):
    """Standard PyTorch implementation"""
    return torch.sign(torch.abs(x))

# Triton kernel - fused element-wise absolute value + Sign
@triton.jit
def abs_sign_kernel(
    x_ptr,
    output_ptr,
    ...
    result = tl.sign(tl.abs(x))
    
    # Store result
    tl.store(output_ptr + offsets, result, mask=mask)

def triton_abs_sign(x):
...
def test_correctness():
    ...
    torch_result = torch_abs_sign(x)
    triton_result = triton_abs_sign(x)
