...
def torch_add_hardshrink(x, y, lambd):
    """Standard PyTorch implementation"""
    return torch.nn.functional.hardshrink(x + y, lambd=lambd)

# Triton kernel - fused element-wise Addition + Hardshrink
@triton.jit
def add_hardshrink_kernel(
...
def triton_add_hardshrink(x, y, lambd):
...
def test_correctness():
    ...
    torch_result = torch_add_hardshrink(x, y, lambd)
    triton_result = triton_add_hardshrink(x, y, lambd)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_add_hardshrink(x, y, lambd)
            _ = triton_add_hardshrink(x, y, lambd)
