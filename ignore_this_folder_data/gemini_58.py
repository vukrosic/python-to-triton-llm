...
def torch_exp_square(x):
    """Standard PyTorch implementation"""
    return (torch.exp(x)) * (torch.exp(x))

# Triton kernel - fused element-wise Exponential + Square
@triton.jit
def exp_square_kernel(
    x_ptr, output_ptr,
    n_elements,
...
def test_correctness():
    ...
    torch_result = torch_exp_square(x)
    triton_result = triton_exp_square(x)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_exp_square(x)
            _ = triton_exp_square(x)
