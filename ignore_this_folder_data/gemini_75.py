...
def torch_sin_square(x):
    """Standard PyTorch implementation"""
    return torch.sin(x) * torch.sin(x)

# Triton kernel - fused element-wise Sine + Square
@triton.jit
def sin_square_kernel(
    x_ptr,
    output_ptr,
    n_elements,
...
def test_correctness():
    ...
    torch_result = torch_sin_square(x)
    triton_result = triton_sin_square(x)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_sin_square(x)
            _ = triton_sin_square(x)
