# triton_gelu_silu_rms_norm_gelu_silu_softmax.py
...
def torch_gelu_silu_rms_norm_gelu_silu_softmax(x, weight, eps):
    \"\"\"Standard PyTorch implementation: softmax(gelu(rms_norm(gelu(x) * silu(x))) * silu(rms_norm(gelu(x) * silu(x))))\"\"\"
    x_gelu = torch.nn.functional.gelu(x)
    x_silu = torch.nn.functional.silu(x)
    x_combined = x_gelu * x_silu
    variance = x_combined.pow(2).mean(-1, keepdim=True)
    x_norm = x_combined * torch.rsqrt(variance + eps)
    x_rms = weight * x_norm
    x_gelu2 = torch.nn.functional.gelu(x_rms)
    x_silu2 = torch.nn.functional.silu(x_rms)
    x_combined2 = x_gelu2 * x_silu2
    return torch.softmax(x_combined2, dim=-1)

# Triton kernel - fused GELU * SiLU + RMS normalization + GELU * SiLU + softmax
@triton.jit
def gelu_silu_rms_norm_gelu_silu_softmax_kernel(
...
def triton_gelu_silu_rms_norm_gelu_silu_softmax(x, weight, eps=1e-6):
    ...
    # Launch kernel
    gelu_silu_rms_norm_gelu_silu_softmax_kernel[grid](
        x, weight, output,
        n_cols, eps,
...
def test_correctness():
    ...
    torch_result = torch_gelu_silu_rms_norm_gelu_silu_softmax(x, weight, eps)
    triton_result = triton_gelu_silu_rms_norm_gelu_silu_softmax(x, weight, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_gelu_silu_rms_norm_gelu_silu_softmax(x, weight, eps)
            _ = triton_gelu_silu_rms_norm_gelu_silu_softmax(x, weight, eps)
