# triton_add_mul_div_sub_gelu_rms_norm.py
...
def torch_add_mul_div_sub_gelu_rms_norm(x, y, z, w, v, weight, eps):
...
# Triton kernel - fused add + mul + div + sub + GELU + RMS normalization
@triton.jit
def add_mul_div_sub_gelu_rms_norm_kernel(
...
def triton_add_mul_div_sub_gelu_rms_norm(x, y, z, w, v, weight, eps=1e-6):
    ...
    # Launch kernel
    add_mul_div_sub_gelu_rms_norm_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_add_mul_div_sub_gelu_rms_norm(x, y, z, w, v, weight, eps)
    triton_result = triton_add_mul_div_sub_gelu_rms_norm(x, y, z, w, v, weight, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_add_mul_div_sub_gelu_rms_norm(x, y, z, w, v, weight, eps)
            _ = triton_add_mul_div_sub_gelu_rms_norm(x, y, z, w, v, weight, eps)
