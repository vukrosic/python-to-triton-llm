...
def torch_add_rms_norm(x, residual, weight, eps):
    """Standard PyTorch implementation: rms_norm(x + residual)"""
    x_res = x + residual
    variance = x_res.pow(2).mean(-1, keepdim=True)
    x_norm = x_res * torch.rsqrt(variance + eps)
    return weight * x_norm

# Triton kernel - fused add + RMS normalization
@triton.jit
...
def triton_add_rms_norm(x, residual, weight, eps=1e-6):
...
def test_correctness():
    ...
    residual = torch.randn(32, 256, device='cuda', dtype=torch.float32)
    weight = torch.randn(256, device='cuda', dtype=torch.float32)
    eps = 1e-6
    
    torch_result = torch_add_rms_norm(x, residual, weight, eps)
    triton_result = triton_add_rms_norm(x, residual, weight, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_add_rms_norm(x, residual, weight, eps)
            _ = triton_add_rms_norm(x, residual, weight, eps)
