# triton_sub_div_mul_add_layer_norm.py
...
def torch_sub_div_mul_add_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps):
    """Standard PyTorch implementation: layer_norm((x - y) / z * w + v)"""
    intermediate = (x - y) / z * w + v
    return torch.nn.functional.layer_norm(intermediate, normalized_shape, weight, bias, eps)

# Triton kernel - fused sub + div + mul + add + layer normalization
@triton.jit
def sub_div_mul_add_layer_norm_kernel(
    ...
    # Layer norm
    mean = tl.sum(intermediate, axis=0) / n_cols
    var = tl.sum((intermediate - mean) * (intermediate - mean), axis=0) / n_cols
    rstd = tl.rsqrt(var + eps)
    
    weight = tl.load(weight_ptr + col_offsets, mask=mask, other=0.0)
    bias = tl.load(bias_ptr + col_offsets, mask=mask, other=0.0)
    
    result = (intermediate - mean) * rstd * weight + bias
    
    # Store result
    tl.store(output_ptr + row_offset + col_offsets, result, mask=mask)

def triton_sub_div_mul_add_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps=1e-5):
    ...
    # Launch kernel
    sub_div_mul_add_layer_norm_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_sub_div_mul_add_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
    triton_result = triton_sub_div_mul_add_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_sub_div_mul_add_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
            _ = triton_sub_div_mul_add_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
