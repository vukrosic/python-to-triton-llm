# triton_add_mul_sub_div_rms_norm.py
...
def torch_add_mul_sub_div_rms_norm(x, y, z, w, v, weight, eps):
    """Standard PyTorch implementation: rms_norm((x + y) * z - w / v)"""
    intermediate = (x + y) * z - w / v
    variance = intermediate.pow(2).mean(-1, keepdim=True)
    x_norm = intermediate * torch.rsqrt(variance + eps)
    return weight * x_norm

# Triton kernel - fused add + mul + sub + div + RMS normalization
@triton.jit
def add_mul_sub_div_rms_norm_kernel(
...
def triton_add_mul_sub_div_rms_norm(x, y, z, w, v, weight, eps=1e-6):
    ...
    # Launch kernel
    add_mul_sub_div_rms_norm_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_add_mul_sub_div_rms_norm(x, y, z, w, v, weight, eps)
    triton_result = triton_add_mul_sub_div_rms_norm(x, y, z, w, v, weight, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_add_mul_sub_div_rms_norm(x, y, z, w, v, weight, eps)
            _ = triton_add_mul_sub_div_rms_norm(x, y, z, w, v, weight, eps)
