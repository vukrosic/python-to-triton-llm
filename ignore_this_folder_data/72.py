# triton_add_sub_mul_div_relu.py
...
def torch_add_sub_mul_div_relu(x, y, z, w, v):
    \"\"\"Standard PyTorch implementation: relu((x + y - z) * w / v)\"\"\"
    intermediate = (x + y - z) * w / v
    return torch.nn.functional.relu(intermediate)

# Triton kernel - fused add + sub + mul + div + ReLU
@triton.jit
def add_sub_mul_div_relu_kernel(
    ...
    # Apply ReLU
    result = tl.maximum(intermediate, 0.0)
    
    # Store result
    tl.store(output_ptr + offsets, result, mask=mask)

def triton_add_sub_mul_div_relu(x, y, z, w, v):
    ...
    # Launch kernel
    add_sub_mul_div_relu_kernel[grid](
        x, y, z, w, v, output,
        n_elements,
...
def test_correctness():
    ...
    v = torch.randn(256, 512, device='cuda', dtype=torch.float32) + 1.0  # Ensure no division by zero
    
    torch_result = torch_add_sub_mul_div_relu(x, y, z, w, v)
    triton_result = triton_add_sub_mul_div_relu(x, y, z, w, v)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_add_sub_mul_div_relu(x, y, z, w, v)
            _ = triton_add_sub_mul_div_relu(x, y, z, w, v)
