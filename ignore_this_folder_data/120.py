# triton_sub_mul_div_add_silu_layer_norm.py
...
def torch_sub_mul_div_add_silu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps):
...
# Triton kernel - fused sub + mul + div + add + SiLU + layer normalization
@triton.jit
def sub_mul_div_add_silu_layer_norm_kernel(
...
def triton_sub_mul_div_add_silu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps=1e-5):
    ...
    # Launch kernel
    sub_mul_div_add_silu_layer_norm_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_sub_mul_div_add_silu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
    triton_result = triton_sub_mul_div_add_silu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_sub_mul_div_add_silu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
            _ = triton_sub_mul_div_add_silu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
