...
def torch_sub_silu(x, y):
    """Standard PyTorch implementation"""
    return torch.nn.functional.silu(x - y)

# Triton kernel - fused element-wise Subtraction + SiLU
@triton.jit
def sub_silu_kernel(
...
def test_correctness():
    ...
    torch_result = torch_sub_silu(x, y)
    triton_result = triton_sub_silu(x, y)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_sub_silu(x, y)
            _ = triton_sub_silu(x, y)
