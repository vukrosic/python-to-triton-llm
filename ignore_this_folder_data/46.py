# triton_layer_norm_gelu_softmax.py
...
def torch_layer_norm_gelu_softmax(x, normalized_shape, weight, bias, eps):
    """Standard PyTorch implementation: softmax(gelu(layer_norm(x)))"""
    # Layer normalization
    x_norm = torch.nn.functional.layer_norm(x, normalized_shape, weight, bias, eps)
    # GELU activation
    x_gelu = torch.nn.functional.gelu(x_norm)
    # Softmax
    return torch.softmax(x_gelu, dim=-1)

# Triton kernel - fused layer norm + GELU + softmax
@triton.jit
def layer_norm_gelu_softmax_kernel(
    ...
    x_scaled = x_norm * 0.7071067811865476  # 1/sqrt(2)
    x_erf = tl.libdevice.erf(x_scaled)
    x_gelu = 0.5 * x_norm * (1 + x_erf)
    
    # Softmax activation
    x_minus_max = x_gelu - tl.max(x_gelu, axis=0)
...
def triton_layer_norm_gelu_softmax(x, normalized_shape, weight, bias, eps=1e-5):
    ...
    # Launch kernel
    layer_norm_gelu_softmax_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_layer_norm_gelu_softmax(x, normalized_shape, weight, bias, eps)
    triton_result = triton_layer_norm_gelu_softmax(x, normalized_shape, weight, bias, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_layer_norm_gelu_softmax(x, normalized_shape, weight, bias, eps)
            _ = triton_layer_norm_gelu_softmax(x, normalized_shape, weight, bias, eps)
