...
def torch_sqrt_exp(x):
    """Standard PyTorch implementation"""
    return torch.exp(torch.sqrt(x))

# Triton kernel - fused element-wise Square Root + Exponential
@triton.jit
def sqrt_exp_kernel(
    x_ptr,
    output_ptr,
    ...
    result = tl.exp(tl.sqrt(x))
    
    # Store result
    tl.store(output_ptr + offsets, result, mask=mask)

def triton_sqrt_exp(x):
...
def test_correctness():
    ...
    torch_result = torch_sqrt_exp(x)
    triton_result = triton_sqrt_exp(x)
