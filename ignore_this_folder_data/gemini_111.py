...
def torch_add_elu(x, y, alpha):
    """Standard PyTorch implementation"""
    return torch.nn.functional.elu(x + y, alpha=alpha)

# Triton kernel - fused element-wise Addition + ELU
@triton.jit
def add_elu_kernel(
...
def test_correctness():
    ...
    torch_result = torch_add_elu(x, y, alpha)
    triton_result = triton_add_elu(x, y, alpha)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_add_elu(x, y, alpha)
            _ = triton_add_elu(x, y, alpha)
