...
def torch_div_sqrt(x, y):
    """Standard PyTorch implementation"""
    return torch.sqrt(x / y)

# Triton kernel - fused element-wise division + Square Root
@triton.jit
def div_sqrt_kernel(
...
def test_correctness():
    ...
    x = torch.randn(1024, 1024, device='cuda', dtype=torch.float32).abs() + 1e-3 # Ensure positive for sqrt
    y = torch.randn(1024, 1024, device='cuda', dtype=torch.float32).abs() + 1e-3 # Ensure positive for division and sqrt
    
    torch_result = torch_div_sqrt(x, y)
    triton_result = triton_div_sqrt(x, y)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_div_sqrt(x, y)
            _ = triton_div_sqrt(x, y)
