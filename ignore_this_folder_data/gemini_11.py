...
def torch_add_tanh(x, y):
    """Standard PyTorch implementation"""
    return torch.tanh(x + y)

# Triton kernel - fused element-wise addition + Tanh
@triton.jit
def add_tanh_kernel(
...
def test_correctness():
    ...
    torch_result = torch_add_tanh(x, y)
    triton_result = triton_add_tanh(x, y)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_add_tanh(x, y)
            _ = triton_add_tanh(x, y)
