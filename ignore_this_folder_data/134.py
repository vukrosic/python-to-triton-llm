# triton_mul_add_sub_div_gelu_layer_norm.py
...
def torch_mul_add_sub_div_gelu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps):
...
# Triton kernel - fused mul + add + sub + div + GELU + layer normalization
@triton.jit
def mul_add_sub_div_gelu_layer_norm_kernel(
...
def triton_mul_add_sub_div_gelu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps=1e-5):
    ...
    # Launch kernel
    mul_add_sub_div_gelu_layer_norm_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_mul_add_sub_div_gelu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
    triton_result = triton_mul_add_sub_div_gelu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_mul_add_sub_div_gelu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
            _ = triton_mul_add_sub_div_gelu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
