...
def torch_silu_rms_norm(x, weight, eps):
    \"\"\"Standard PyTorch implementation: rms_norm(silu(x))\"\"\"
    x_silu = torch.nn.functional.silu(x)
    variance = x_silu.pow(2).mean(-1, keepdim=True)
    x_norm = x_silu * torch.rsqrt(variance + eps)
    return weight * x_norm

# Triton kernel - fused SiLU + RMS normalization
@triton.jit
def silu_rms_norm_kernel(
...
def triton_silu_rms_norm(x, weight, eps=1e-6):
...
def test_correctness():
    ...
    torch_result = torch_silu_rms_norm(x, weight, eps)
    triton_result = triton_silu_rms_norm(x, weight, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_silu_rms_norm(x, weight, eps)
            _ = triton_silu_rms_norm(x, weight, eps)
