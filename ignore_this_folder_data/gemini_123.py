...
def torch_add_softmax(x, y):
    """Standard PyTorch implementation"""
    return torch.nn.functional.softmax(x + y, dim=-1)

# Triton kernel - fused element-wise Addition + Softmax
@triton.jit
...
def test_correctness():
    ...
    torch_result = torch_add_softmax(x, y)
    triton_result = triton_add_softmax(x, y)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_add_softmax(x, y)
            _ = triton_add_softmax(x, y)
