...
def torch_mul_add_sub_gelu(x, y, z, w):
    \"\"\"Standard PyTorch implementation: gelu(x * y + z - w)\"\"\"
    intermediate = x * y + z - w
    return torch.nn.functional.gelu(intermediate)

# Triton kernel - fused mul + add + sub + GELU
@triton.jit
def mul_add_sub_gelu_kernel(
...
def triton_mul_add_sub_gelu(x, y, z, w):
...
def test_correctness():
    ...
    torch_result = torch_mul_add_sub_gelu(x, y, z, w)
    triton_result = triton_mul_add_sub_gelu(x, y, z, w)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_mul_add_sub_gelu(x, y, z, w)
            _ = triton_mul_add_sub_gelu(x, y, z, w)
