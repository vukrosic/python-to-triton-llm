# triton_gelu_silu_rms_norm_gelu_softmax.py
...
def torch_gelu_silu_rms_norm_gelu_softmax(x, weight, eps):
    """Standard PyTorch implementation: softmax(gelu(rms_norm(gelu(x) * silu(x))))"""
...
# Triton kernel - fused GELU * SiLU + RMS normalization + GELU + softmax
@triton.jit
def gelu_silu_rms_norm_gelu_softmax_kernel(
    ...
    combined = x_gelu1 * x_silu
    
    # Compute RMS normalization
    x_squared = combined * combined
...
def triton_gelu_silu_rms_norm_gelu_softmax(x, weight, eps=1e-6):
    ...
    # Launch kernel
    gelu_silu_rms_norm_gelu_softmax_kernel[grid](
        x, weight, output,
        n_cols, eps,
...
def test_correctness():
    ...
    torch_result = torch_gelu_silu_rms_norm_gelu_softmax(x, weight, eps)
    triton_result = triton_gelu_silu_rms_norm_gelu_softmax(x, weight, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_gelu_silu_rms_norm_gelu_softmax(x, weight, eps)
            _ = triton_gelu_silu_rms_norm_gelu_softmax(x, weight, eps)
