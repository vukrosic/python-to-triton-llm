...
def torch_add_pow(x, y, exponent):
    """Standard PyTorch implementation"""
    return torch.pow(x + y, exponent)

# Triton kernel - fused element-wise Addition + Power
@triton.jit
def add_pow_kernel(
    ...
    result = tl.pow(x + y, exponent)
    
    # Store result
    tl.store(output_ptr + offsets, result, mask=mask)

def triton_add_pow(x, y, exponent):
...
def test_correctness():
    ...
    torch_result = torch_add_pow(x, y, exponent)
    triton_result = triton_add_pow(x, y, exponent)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_add_pow(x, y, exponent)
            _ = triton_add_pow(x, y, exponent)
