...
def torch_matmul_add_relu(x, w, bias):
    \"\"\"Standard PyTorch implementation: ReLU(matmul(x, w) + bias)\"\"\"
    return torch.nn.functional.relu(torch.matmul(x, w) + bias)
...
def triton_matmul_add_relu(x, w, bias):
...
def test_correctness():
    ...
    torch_result = torch_matmul_add_relu(x, w, bias)
    triton_result = triton_matmul_add_relu(x, w, bias)
...
def benchmark():
    ...
    for M, K, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_matmul_add_relu(x, w, bias)
            _ = triton_matmul_add_relu(x, w, bias)
