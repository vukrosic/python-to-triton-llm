...
def torch_sqrt_tan(x):
    """Standard PyTorch implementation"""
    return torch.tan(torch.sqrt(x))

# Triton kernel - fused element-wise Square Root + Tangent
@triton.jit
def sqrt_tan_kernel(
    x_ptr,
    output_ptr,
    ...
    result = tl.tan(tl.sqrt(x))
    
    # Store result
    tl.store(output_ptr + offsets, result, mask=mask)

def triton_sqrt_tan(x):
...
def test_correctness():
    ...
    torch_result = torch_sqrt_tan(x)
    triton_result = triton_sqrt_tan(x)
