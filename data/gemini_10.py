...
def torch_mul_sigmoid(x, y):
    """Standard PyTorch implementation"""
    return torch.sigmoid(x * y)

# Triton kernel - fused element-wise multiplication + Sigmoid
@triton.jit
def mul_sigmoid_kernel(
...
def test_correctness():
    ...
    torch_result = torch_mul_sigmoid(x, y)
    triton_result = triton_mul_sigmoid(x, y)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_mul_sigmoid(x, y)
            _ = triton_mul_sigmoid(x, y)
