# triton_rms_norm_gelu_softmax.py
...
def torch_rms_norm_gelu_softmax(x, weight, eps):
    \"\"\"Standard PyTorch implementation: softmax(gelu(rms_norm(x)))\"\"\"
    # RMS normalization
...
# Triton kernel - fused RMS normalization + GELU + softmax
@triton.jit
def rms_norm_gelu_softmax_kernel(
    ...
    x_scaled = x_rms * 0.7071067811865476  # 1/sqrt(2)
    x_erf = tl.libdevice.erf(x_scaled)
    x_gelu = 0.5 * x_rms * (1 + x_erf)
...
def triton_rms_norm_gelu_softmax(x, weight, eps=1e-6):
    ...
    # Launch kernel
    rms_norm_gelu_softmax_kernel[grid](
        x, weight, output,
        n_cols, eps,
...
def test_correctness():
    ...
    torch_result = torch_rms_norm_gelu_softmax(x, weight, eps)
    triton_result = triton_rms_norm_gelu_softmax(x, weight, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_rms_norm_gelu_softmax(x, weight, eps)
            _ = triton_rms_norm_gelu_softmax(x, weight, eps)
