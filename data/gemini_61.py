...
def torch_exp_cube(x):
    """Standard PyTorch implementation"""
    return torch.pow(torch.exp(x), 3)

# Triton kernel - fused element-wise Exponential + Cube
@triton.jit
def exp_cube_kernel(
    x_ptr,
    output_ptr,
    ...
    exp_x = tl.exp(x)
    result = tl.pow(exp_x, 3)
    
    # Store result
    tl.store(output_ptr + offsets, result, mask=mask)

def triton_exp_cube(x):
...
def test_correctness():
    ...
    torch_result = torch_exp_cube(x)
    triton_result = triton_exp_cube(x)
