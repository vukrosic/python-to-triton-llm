...
def torch_gelu_silu_softmax(x):
    \"\"\"Standard PyTorch implementation: softmax(gelu(x) * silu(x))\"\"\"
...
# Triton kernel - fused GELU * SiLU + softmax
@triton.jit
def gelu_silu_softmax_kernel(
...
def triton_gelu_silu_softmax(x):
    \"\"\"Triton wrapper for GELU * SiLU with softmax\"\"\"
    # Ensure input is 2D
    if x.dim() == 1:
        x = x.unsqueeze(0)
...
def test_correctness():
    ...
    for rows, cols in test_sizes:
        ...
        torch_result = torch_gelu_silu_softmax(x)
        triton_result = triton_gelu_silu_softmax(x)
...
def benchmark():
    ...
    for rows, cols in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_gelu_silu_softmax(x)
            _ = triton_gelu_silu_softmax(x)
