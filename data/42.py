# triton_gelu_layer_norm_softmax.py
...
def torch_gelu_layer_norm_softmax(x, normalized_shape, weight, bias, eps):
    """Standard PyTorch implementation: softmax(layer_norm(gelu(x)))"""
    x_gelu = torch.nn.functional.gelu(x)
    x_norm = torch.nn.functional.layer_norm(x_gelu, normalized_shape, weight, bias, eps)
    return torch.softmax(x_norm, dim=-1)

# Triton kernel - fused GELU + layer norm + softmax
@triton.jit
def gelu_layer_norm_softmax_kernel(
    ...
    # Layer norm
    mean = tl.sum(gelu_x, axis=0) / n_cols
    var = tl.sum((gelu_x - mean) * (gelu_x - mean), axis=0) / n_cols
    rstd = tl.rsqrt(var + eps)
    
    weight = tl.load(weight_ptr + col_offsets, mask=mask, other=0.0)
    bias = tl.load(bias_ptr + col_offsets, mask=mask, other=0.0)
    
    x_norm = (gelu_x - mean) * rstd * weight + bias
    
    # Softmax activation
    x_minus_max = x_norm - tl.max(x_norm, axis=0)
...
def triton_gelu_layer_norm_softmax(x, normalized_shape, weight, bias, eps=1e-5):
    ...
    # Launch kernel
    gelu_layer_norm_softmax_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_gelu_layer_norm_softmax(x, normalized_shape, weight, bias, eps)
    triton_result = triton_gelu_layer_norm_softmax(x, normalized_shape, weight, bias, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_gelu_layer_norm_softmax(x, normalized_shape, weight, bias, eps)
            _ = triton_gelu_layer_norm_softmax(x, normalized_shape, weight, bias, eps)
