...
def torch_sub_softmax(x, y):
    """Standard PyTorch implementation"""
    return torch.nn.functional.softmax(x - y, dim=-1)

# Triton kernel - fused element-wise Subtraction + Softmax
@triton.jit
def sub_softmax_kernel(
...
def test_correctness():
    ...
    torch_result = torch_sub_softmax(x, y)
    triton_result = triton_sub_softmax(x, y)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_sub_softmax(x, y)
            _ = triton_sub_softmax(x, y)
