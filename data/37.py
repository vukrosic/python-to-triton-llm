...
def torch_softmax_silu(x):
    \"\"\"Standard PyTorch implementation: silu(softmax(x))\"\"\"
    x_softmax = torch.softmax(x, dim=-1)
    return torch.nn.functional.silu(x_softmax)

# Triton kernel - fused softmax + SiLU
@triton.jit
def softmax_silu_kernel(
    ...
    x_sigmoid = tl.sigmoid(softmax_output)
    result = softmax_output * x_sigmoid
    
    # Store result
    output_ptrs = output_row_start_ptr + col_offsets
    tl.store(output_ptrs, result, mask=mask)

def triton_softmax_silu(x):
    \"\"\"Triton wrapper for softmax with SiLU\"\"\"
    # Ensure input is 2D
    if x.dim() == 1:
        x = x.unsqueeze(0)
...
def test_correctness():
    ...
    for rows, cols in test_sizes:
        ...
        torch_result = torch_softmax_silu(x)
        triton_result = triton_softmax_silu(x)
...
def benchmark():
    ...
    for rows, cols in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_softmax_silu(x)
            _ = triton_softmax_silu(x)
