...
def torch_square_exp(x):
    """Standard PyTorch implementation"""
    return torch.exp(x * x)

# Triton kernel - fused element-wise Square + Exponential
@triton.jit
def square_exp_kernel(
    x_ptr,
    output_ptr,
    n_elements,
...
def test_correctness():
    ...
    torch_result = torch_square_exp(x)
    triton_result = triton_square_exp(x)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_square_exp(x)
            _ = triton_square_exp(x)
