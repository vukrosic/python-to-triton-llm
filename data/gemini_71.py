...
# Triton kernel - fused element-wise Tangent + Exponential
@triton.jit
def tan_exp_kernel(
    x_ptr,
    output_ptr,
    ...
    result = tl.exp(tl.tan(x))
    
    # Store result
    tl.store(output_ptr + offsets, result, mask=mask)

def triton_tan_exp(x):
...
def test_correctness():
    ...
    torch_result = torch_tan_exp(x)
    triton_result = triton_tan_exp(x)
