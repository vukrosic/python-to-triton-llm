...
def torch_gelu_softmax(x):
    \"\"\"Standard PyTorch implementation: softmax(gelu(x))\"\"\"
    x_gelu = torch.nn.functional.gelu(x)
    return torch.softmax(x_gelu, dim=-1)

# Triton kernel - fused GELU + softmax
@triton.jit
def gelu_softmax_kernel(
    ...
    # Compute softmax
    x_minus_max = gelu_x - tl.max(gelu_x, axis=0)
...
def triton_gelu_softmax(x):
    \"\"\"Triton wrapper for GELU with softmax\"\"\"
    # Ensure input is 2D
    if x.dim() == 1:
        x = x.unsqueeze(0)
...
def test_correctness():
    ...
    for rows, cols in test_sizes:
        ...
        torch_result = torch_gelu_softmax(x)
        triton_result = triton_gelu_softmax(x)
...
def benchmark():
    ...
    for rows, cols in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_gelu_softmax(x)
            _ = triton_gelu_softmax(x)
