...
def torch_div_elu(x, y, alpha):
    """Standard PyTorch implementation"""
    return torch.nn.functional.elu(x / y, alpha=alpha)

# Triton kernel - fused element-wise Division + ELU
@triton.jit
def div_elu_kernel(
...
def test_correctness():
    ...
    torch_result = torch_div_elu(x, y, alpha)
    triton_result = triton_div_elu(x, y, alpha)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_div_elu(x, y, alpha)
            _ = triton_div_elu(x, y, alpha)
