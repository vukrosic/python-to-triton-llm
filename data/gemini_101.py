...
def torch_div_gelu(x, y):
    """Standard PyTorch implementation"""
    return torch.nn.functional.gelu(x / y)

# Triton kernel - fused element-wise Division + GELU
@triton.jit
def div_gelu_kernel(
...
def test_correctness():
    ...
    torch_result = torch_div_gelu(x, y)
    triton_result = triton_div_gelu(x, y)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_div_gelu(x, y)
            _ = triton_div_gelu(x, y)
