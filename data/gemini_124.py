...
def torch_mul_softmax(x, y):
    """Standard PyTorch implementation"""
    return torch.nn.functional.softmax(x * y, dim=-1)

# Triton kernel - fused element-wise Multiplication + Softmax
@triton.jit
def mul_softmax_kernel(
...
def test_correctness():
    ...
    torch_result = torch_mul_softmax(x, y)
    triton_result = triton_mul_softmax(x, y)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_mul_softmax(x, y)
            _ = triton_mul_softmax(x, y)
