# triton_sub_div_mul_add_gelu_layer_norm.py
...
def torch_sub_div_mul_add_gelu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps):
...
# Triton kernel - fused sub + div + mul + add + GELU + layer normalization
@triton.jit
def sub_div_mul_add_gelu_layer_norm_kernel(
...
def triton_sub_div_mul_add_gelu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps=1e-5):
    ...
    # Launch kernel
    sub_div_mul_add_gelu_layer_norm_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_sub_div_mul_add_gelu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
    triton_result = triton_sub_div_mul_add_gelu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_sub_div_mul_add_gelu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
            _ = triton_sub_div_mul_add_gelu_layer_norm(x, y, z, w, v, normalized_shape, weight, bias, eps)
