# triton_layer_norm_silu_softmax.py
...
def torch_layer_norm_silu_softmax(x, normalized_shape, weight, bias, eps):
    """Standard PyTorch implementation: softmax(silu(layer_norm(x)))"""
    # Layer normalization
    x_norm = torch.nn.functional.layer_norm(x, normalized_shape, weight, bias, eps)
    # SiLU activation
    x_silu = torch.nn.functional.silu(x_norm)
    # Softmax
    return torch.softmax(x_silu, dim=-1)

# Triton kernel - fused layer norm + SiLU + softmax
@triton.jit
def layer_norm_silu_softmax_kernel(
    ...
    x_silu = x_norm * x_sigmoid
    
    # Softmax activation
    x_minus_max = x_silu - tl.max(x_silu, axis=0)
...
def triton_layer_norm_silu_softmax(x, normalized_shape, weight, bias, eps=1e-5):
    ...
    # Launch kernel
    layer_norm_silu_softmax_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_layer_norm_silu_softmax(x, normalized_shape, weight, bias, eps)
    triton_result = triton_layer_norm_silu_softmax(x, normalized_shape, weight, bias, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_layer_norm_silu_softmax(x, normalized_shape, weight, bias, eps)
            _ = triton_layer_norm_silu_softmax(x, normalized_shape, weight, bias, eps)
