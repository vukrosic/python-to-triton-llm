...
def torch_div_softmax(x, y):
    """Standard PyTorch implementation"""
    return torch.nn.functional.softmax(x / y, dim=-1)

# Triton kernel - fused element-wise Division + Softmax
@triton.jit
def div_softmax_kernel(
...
def test_correctness():
    ...
    torch_result = torch_div_softmax(x, y)
    triton_result = triton_div_softmax(x, y)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_div_softmax(x, y)
            _ = triton_div_softmax(x, y)
