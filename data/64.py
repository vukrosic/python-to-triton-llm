...
def torch_add_mul_rms_norm(x, y, z, weight, eps):
    \"\"\"Standard PyTorch implementation: rms_norm((x + y) * z)\"\"\"
    intermediate = (x + y) * z
    variance = intermediate.pow(2).mean(-1, keepdim=True)
    x_norm = intermediate * torch.rsqrt(variance + eps)
    return weight * x_norm

# Triton kernel - fused add + mul + RMS normalization
@triton.jit
def add_mul_rms_norm_kernel(
    x_ptr, y_ptr, z_ptr, weight_ptr, output_ptr,
    n_cols, eps,
...
def triton_add_mul_rms_norm(x, y, z, weight, eps=1e-6):
...
def test_correctness():
    ...
    torch_result = torch_add_mul_rms_norm(x, y, z, weight, eps)
    triton_result = triton_add_mul_rms_norm(x, y, z, weight, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_add_mul_rms_norm(x, y, z, weight, eps)
            _ = triton_add_mul_rms_norm(x, y, z, weight, eps)
