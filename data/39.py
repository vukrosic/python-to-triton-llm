...
def torch_layer_norm_softmax(x, normalized_shape, weight, bias, eps):
    """Standard PyTorch implementation: softmax(layer_norm(x))"""
    x_norm = torch.nn.functional.layer_norm(x, normalized_shape, weight, bias, eps)
    return torch.softmax(x_norm, dim=-1)

# Triton kernel - fused layer norm + softmax
@triton.jit
def layer_norm_softmax_kernel(
...
def triton_layer_norm_softmax(x, normalized_shape, weight, bias, eps=1e-5):
    ...
    # Launch kernel
    layer_norm_softmax_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_layer_norm_softmax(x, normalized_shape, weight, bias, eps)
    triton_result = triton_layer_norm_softmax(x, normalized_shape, weight, bias, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_layer_norm_softmax(x, normalized_shape, weight, bias, eps)
            _ = triton_layer_norm_softmax(x, normalized_shape, weight, bias, eps)
