...
def torch_exp_tan(x):
    """Standard PyTorch implementation"""
    return torch.tan(torch.exp(x))

# Triton kernel - fused element-wise Exponential + Tangent
@triton.jit
def exp_tan_kernel(
    x_ptr,
    output_ptr,
    ...
    result = tl.tan(tl.exp(x))
    
    # Store result
    tl.store(output_ptr + offsets, result, mask=mask)

def triton_exp_tan(x):
...
def test_correctness():
    ...
    torch_result = torch_exp_tan(x)
    triton_result = triton_exp_tan(x)
