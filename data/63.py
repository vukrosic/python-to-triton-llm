# triton_layer_norm_gelu_silu_layer_norm_silu_softmax.py
...
def torch_layer_norm_gelu_silu_layer_norm_silu_softmax(x, normalized_shape, weight1, bias1, weight2, bias2, eps):
    """Standard PyTorch implementation: softmax(silu(layer_norm(gelu(layer_norm(x)) * silu(layer_norm(x)))))"""
    ...
    # SiLU activation
    x_silu2 = torch.nn.functional.silu(x_norm2)
    
    # Softmax
    return torch.softmax(x_silu2, dim=-1)

# Triton kernel - fused layer norm + GELU * SiLU + layer norm + SiLU + softmax
@triton.jit
def layer_norm_gelu_silu_layer_norm_silu_softmax_kernel(
    ...
    x_sigmoid_silu2 = tl.sigmoid(x_norm2)
    x_silu2 = x_norm2 * x_sigmoid_silu2
...
def triton_layer_norm_gelu_silu_layer_norm_silu_softmax(x, normalized_shape, weight1, bias1, weight2, bias2, eps=1e-5):
    ...
    # Launch kernel
    layer_norm_gelu_silu_layer_norm_silu_softmax_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_layer_norm_gelu_silu_layer_norm_silu_softmax(x, normalized_shape, weight1, bias1, weight2, bias2, eps)
    triton_result = triton_layer_norm_gelu_silu_layer_norm_silu_softmax(x, normalized_shape, weight1, bias1, weight2, bias2, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_layer_norm_gelu_silu_layer_norm_silu_softmax(x, normalized_shape, weight1, bias1, weight2, bias2, eps)
            _ = triton_layer_norm_gelu_silu_layer_norm_silu_softmax(x, normalized_shape, weight1, bias1, weight2, bias2, eps)
