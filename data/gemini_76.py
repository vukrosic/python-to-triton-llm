...
def torch_cos_square(x):
    """Standard PyTorch implementation"""
    return torch.cos(x) * torch.cos(x)

# Triton kernel - fused element-wise Cosine + Square
@triton.jit
def cos_square_kernel(
    x_ptr,
    output_ptr,
    n_elements,
...
def test_correctness():
    ...
    torch_result = torch_cos_square(x)
    triton_result = triton_cos_square(x)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_cos_square(x)
            _ = triton_cos_square(x)
