# triton_rms_norm_gelu_silu_rms_norm_softmax.py
...
def torch_rms_norm_gelu_silu_rms_norm_softmax(x, weight1, weight2, eps):
    """Standard PyTorch implementation: softmax(rms_norm(gelu(rms_norm(x)) * silu(rms_norm(x))))"""
    ...
    x_gelu = torch.nn.functional.gelu(x_rms1)
    x_silu = torch.nn.functional.silu(x_rms1)
    
    # Elementwise multiplication
    x_combined = x_gelu * x_silu
    
    # Second RMS normalization
    variance2 = x_combined.pow(2).mean(-1, keepdim=True)
    x_norm2 = x_combined * torch.rsqrt(variance2 + eps)
    x_rms2 = weight2 * x_norm2
    
    # Softmax
    return torch.softmax(x_rms2, dim=-1)

# Triton kernel - fused RMS normalization + GELU * SiLU + RMS normalization + softmax
@triton.jit
def rms_norm_gelu_silu_rms_norm_softmax_kernel(
    ...
    x_scaled_gelu = x_rms1 * 0.7071067811865476  # 1/sqrt(2)
    x_erf_gelu = tl.libdevice.erf(x_scaled_gelu)
    x_gelu = 0.5 * x_rms1 * (1 + x_erf_gelu)
    
    # Compute SiLU: x * sigmoid(x)
    x_sigmoid_silu = tl.sigmoid(x_rms1)
    x_silu = x_rms1 * x_sigmoid_silu
    
    # Elementwise multiplication
    combined = x_gelu * x_silu
    
    # Second RMS normalization
    x_squared2 = combined * combined
    mean_square2 = tl.sum(x_squared2, axis=0) / n_cols
    rms2 = tl.sqrt(mean_square2 + eps)
    x_norm2 = combined / rms2
    x_rms2 = x_norm2 * weight2
    
    # Compute softmax
    x_minus_max = x_rms2 - tl.max(x_rms2, axis=0)
...
def triton_rms_norm_gelu_silu_rms_norm_softmax(x, weight1, weight2, eps=1e-6):
    ...
    # Launch kernel
    rms_norm_gelu_silu_rms_norm_softmax_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_rms_norm_gelu_silu_rms_norm_softmax(x, weight1, weight2, eps)
    triton_result = triton_rms_norm_gelu_silu_rms_norm_softmax(x, weight1, weight2, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_rms_norm_gelu_silu_rms_norm_softmax(x, weight1, weight2, eps)
            _ = triton_rms_norm_gelu_silu_rms_norm_softmax(x, weight1, weight2, eps)
