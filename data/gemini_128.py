...
def torch_mul_logsoftmax(x, y):
    """Standard PyTorch implementation"""
    return torch.nn.functional.log_softmax(x * y, dim=-1)

# Triton kernel - fused element-wise Multiplication + LogSoftmax
@triton.jit
def mul_logsoftmax_kernel(
...
def test_correctness():
    ...
    torch_result = torch_mul_logsoftmax(x, y)
    triton_result = triton_mul_logsoftmax(x, y)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_mul_logsoftmax(x, y)
            _ = triton_mul_logsoftmax(x, y)
