...
def torch_sub_logsoftmax(x, y):
    """Standard PyTorch implementation"""
    return torch.nn.functional.log_softmax(x - y, dim=-1)

# Triton kernel - fused element-wise Subtraction + LogSoftmax
@triton.jit
def sub_logsoftmax_kernel(
...
def test_correctness():
    ...
    torch_result = torch_sub_logsoftmax(x, y)
    triton_result = triton_sub_logsoftmax(x, y)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(10):
            _ = torch_sub_logsoftmax(x, y)
            _ = triton_sub_logsoftmax(x, y)
