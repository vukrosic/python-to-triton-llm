# triton_mul_div_add_sub_silu_rms_norm.py
...
def torch_mul_div_add_sub_silu_rms_norm(x, y, z, w, v, weight, eps):
    """Standard PyTorch implementation: rms_norm(silu(x * y / z + w - v))"""
...
# Triton kernel - fused mul + div + add + sub + SiLU + RMS normalization
@triton.jit
def mul_div_add_sub_silu_rms_norm_kernel(
...
def triton_mul_div_add_sub_silu_rms_norm(x, y, z, w, v, weight, eps=1e-6):
    ...
    # Launch kernel
    mul_div_add_sub_silu_rms_norm_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_mul_div_add_sub_silu_rms_norm(x, y, z, w, v, weight, eps)
    triton_result = triton_mul_div_add_sub_silu_rms_norm(x, y, z, w, v, weight, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_mul_div_add_sub_silu_rms_norm(x, y, z, w, v, weight, eps)
            _ = triton_mul_div_add_sub_silu_rms_norm(x, y, z, w, v, weight, eps)
