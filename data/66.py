...
def torch_mul_add_relu(x, y, z):
    \"\"\"Standard PyTorch implementation: relu(x * y + z)\"\"\"
    intermediate = x * y + z
    return torch.nn.functional.relu(intermediate)

# Triton kernel - fused mul + add + ReLU
@triton.jit
def mul_add_relu_kernel(
...
def test_correctness():
    ...
    torch_result = torch_mul_add_relu(x, y, z)
    triton_result = triton_mul_add_relu(x, y, z)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_mul_add_relu(x, y, z)
            _ = triton_mul_add_relu(x, y, z)
