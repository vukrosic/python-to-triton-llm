...
# Triton kernel - fused element-wise Cosine + Exponential
@triton.jit
def cos_exp_kernel(
    x_ptr,
    output_ptr,
    ...
    result = tl.exp(tl.cos(x))
    
    # Store result
    tl.store(output_ptr + offsets, result, mask=mask)

def triton_cos_exp(x):
...
def test_correctness():
    ...
    torch_result = torch_cos_exp(x)
    triton_result = triton_cos_exp(x)
