# triton_layer_norm_gelu_silu_softmax.py
...
def torch_layer_norm_gelu_silu_softmax(x, normalized_shape, weight, bias, eps):
    """Standard PyTorch implementation: softmax(gelu(layer_norm(x)) * silu(layer_norm(x)))"""
    # Layer normalization
    ...
    x_silu = torch.nn.functional.silu(x_norm)
    # Elementwise multiplication
    x_combined = x_gelu * x_silu
    # Softmax
    return torch.softmax(x_combined, dim=-1)

# Triton kernel - fused layer norm + GELU * SiLU + softmax
@triton.jit
def layer_norm_gelu_silu_softmax_kernel(
    ...
    x_scaled_gelu = x_norm * 0.7071067811865476  # 1/sqrt(2)
    x_erf_gelu = tl.libdevice.erf(x_scaled_gelu)
    x_gelu = 0.5 * x_norm * (1 + x_erf_gelu)
    
    # Compute SiLU: x * sigmoid(x)
    x_sigmoid_silu = tl.sigmoid(x_norm)
    x_silu = x_norm * x_sigmoid_silu
    
    # Elementwise multiplication
    combined = x_gelu * x_silu
    
    # Softmax activation
    x_minus_max = combined - tl.max(combined, axis=0)
...
def triton_layer_norm_gelu_silu_softmax(x, normalized_shape, weight, bias, eps=1e-5):
    ...
    # Launch kernel
    layer_norm_gelu_silu_softmax_kernel[grid](
...
def test_correctness():
    ...
    torch_result = torch_layer_norm_gelu_silu_softmax(x, normalized_shape, weight, bias, eps)
    triton_result = triton_layer_norm_gelu_silu_softmax(x, normalized_shape, weight, bias, eps)
...
def benchmark():
    ...
    for M, N in sizes:
        ...
        # Warmup
        for _ in range(5):
            _ = torch_layer_norm_gelu_silu_softmax(x, normalized_shape, weight, bias, eps)
            _ = triton_layer_norm_gelu_silu_softmax(x, normalized_shape, weight, bias, eps)
