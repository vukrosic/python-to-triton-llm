...
# Triton kernel - fused element-wise Logarithm + Exponential
@triton.jit
def log_exp_kernel(
    x_ptr,
    output_ptr,
    ...
    result = tl.exp(tl.log(x))
    
    # Store result
    tl.store(output_ptr + offsets, result, mask=mask)

def triton_log_exp(x):
...
def test_correctness():
    ...
    torch_result = torch_log_exp(x)
    triton_result = triton_log_exp(x)
